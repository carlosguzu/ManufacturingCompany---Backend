...
def create_order_execution():
    ...
    new_order_execution = OrderExecution(
        number=data['number'],
        bottle_type=data['bottle_type'],
        quantity=data['quantity'],
        status=data['status']
    ...
    order_executions.append(new_order_execution)
...
def get_order_executions():
    order_executions_data = []
    for order_execution in order_executions:
        order_executions_data.append({
            'number': order_execution.number,
            'bottle_type': order_execution.bottle_type,
            'quantity': order_execution.quantity,
            'status': order_execution.status
        })
    return jsonify({'order_executions': order_executions_data})
