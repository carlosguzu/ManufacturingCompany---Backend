...
def create_order(order: OrderCreate):
    ...
    return {"id": order_id, "type": order.bottle_type, "quantity": order.quantity, "status": order.status}
...
def read_order(order_id: int):
    ...
    return {"id": order.id, "type": order.bottle_type, "quantity": order.quantity, "status": order.status}

@app.get("/orders/", response_model=List[OrderResponse])
def read_orders(skip: int = 0, limit: int = 10):
    orders = list(orders_db.values())[skip : skip + limit]
    return [{"id": order.id, "type": order.bottle_type, "quantity": order.quantity, "status": order.status} for order in orders]
